...
Build a custom silver-research-bot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of silver-research-bot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

silver-research-bot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `silver-research-bot gateway` starts, it scans:

1. Built-in channels in `silver-research-bot/channels/`
2. External packages registered under the `silver-research-bot.channels` entry point group
...
```text
silver-research-bot-channel-webhook/
â”œâ”€â”€ silver-research-bot_channel_webhook/
â”?  â”œâ”€â”€ __init__.py          # re-export WebhookChannel
â”?  â””â”€â”€ channel.py           # channel implementation
...
```python
# silver-research-bot_channel_webhook/__init__.py
from silver-research-bot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# silver-research-bot_channel_webhook/channel.py
import asyncio
...
class WebhookChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        msg.content  â€?markdown text (convert to platform format as needed)
        msg.media    â€?list of local file paths to attach
        msg.chat_id  â€?the recipient (same chat_id you passed to _handle_message)
        msg.metadata â€?may contain "_progress": True for streaming chunks
        """
...
# pyproject.toml
[project]
name = "silver-research-bot-channel-webhook"
version = "0.1.0"
dependencies = ["silver-research-bot-ai", "aiohttp"]

[project.entry-points."silver-research-bot.channels"]
webhook = "silver-research-bot_channel_webhook:WebhookChannel"
...
packages = ["silver-research-bot_channel_webhook"]
```
...
```bash
pip install -e .
silver-research-bot plugins list      # verify "Webhook" shows as "plugin"
silver-research-bot onboard           # auto-adds default config for detected plugins
```

Edit `~/.silver-research-bot/config.json`:
...
```bash
silver-research-bot channels login <channel_name>
silver-research-bot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `silver-research-bot onboard`. Override to declare your fields. |
...
class OutboundMessage:
    ...
    content: str        # markdown text â€?convert to platform format as needed
...
`BaseChannel.is_allowed()` reads the permission list via `getattr(self.config, "allow_from", [])`. This works for Pydantic models where `allow_from` is a real Python attribute, but **fails silently for plain `dict`** â€?`dict` has no `allow_from` attribute, so `getattr` always returns the default `[]`, causing all messages to be denied.

Built-in channels use Pydantic config models (subclassing `Base` from `silver-research-bot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `silver-research-bot.config.schema.Base`:
...
2. Convert `dict` â†?model in `__init__`:
...
`allowFrom` is handled automatically by `_handle_message()` â€?you don't need to check it yourself.

Override `default_config()` so `silver-research-bot onboard` auto-populates `config.json`:
...
> **Note:** `default_config()` returns a plain `dict` (not a Pydantic model) because it's used to serialize into `config.json`. The recommended way is to instantiate your config model and call `model_dump(by_alias=True)` â€?this automatically uses camelCase keys (`allowFrom`) and keeps defaults in a single source of truth.
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `silver-research-bot-channel-{name}` | `silver-research-bot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `silver-research-bot_channel_{name}` | `silver-research-bot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/silver-research-bot-channel-webhook
cd silver-research-bot-channel-webhook
pip install -e .
silver-research-bot plugins list    # should show "Webhook" as "plugin"
silver-research-bot gateway         # test end-to-end
```

## Verify

```bash
$ silver-research-bot plugins list

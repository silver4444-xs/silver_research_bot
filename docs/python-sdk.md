...
Use silver-research-bot as a library â€?no CLI, no gateway, just Python.
...
from silver_research_bot import silver_research_bot


async def main() -> None:
    bot = silver-research-bot.from_config()
...
`silver-research-bot.from_config()` reuses your normal `~/.silver-research-bot/config.json`, so the SDK follows the same provider, model, tools, and workspace defaults as the CLI unless you override them.
...
bot = silver-research-bot.from_config(
    config_path="~/.silver-research-bot/config.json",
...
Hooks let you inspect tool calls, streaming, and iteration state without modifying silver-research-bot internals:

```python
from silver_research_bot.agent import AgentHook, AgentHookContext
...
### `silver-research-bot.from_config(config_path=None, *, workspace=None)`

Create a `silver-research-bot` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.silver-research-bot/config.json`. |
...
```python
from silver_research_bot.agent import AgentHook
...
async def main() -> None:
    bot = silver-research-bot.from_config(workspace="/my/project")

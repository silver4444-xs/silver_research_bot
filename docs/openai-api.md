...
silver-research-bot can expose a minimal OpenAI-compatible endpoint for local integrations:

```bash
pip install "silver-research-bot-ai[api]"
silver-research-bot serve
```
...
If `channel` points to a channel that is not enabled in your config, silver-research-bot will queue the outbound event but no platform delivery will occur.

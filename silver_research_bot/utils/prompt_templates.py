"""在 silver_research_bot/templates/ 目录下加载并渲染代理系统提示模板（Jinja2）。

代理提示位于 ``templates/agent/`` 目录下（文件名格式如 ``agent/identity.md``）。
共享副本位于 ``agent/_snippets/`` 目录下，并通过
``{% include ‘agent/_snippets/....md’ %}`` 进行包含。
"""

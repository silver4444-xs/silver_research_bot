...
from silver_research_bot.config.paths import get_media_dir, get_runtime_subdir
from silver_research_bot.config.schema import Base
from silver_research_bot.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.silver_research_bot/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'silver_research_bot channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"silver_research_bot-{uuid.uuid4().hex[:12]}"

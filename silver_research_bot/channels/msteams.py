...
class MSTeamsChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            logger.error("PyJWT not installed. Run: pip install silver_research_bot-ai[msteams]")
            return
        ...
        self._server_thread = threading.Thread(
            ...
            name="silver_research_bot-msteams",
            daemon=True,
        )
    ...
    async def _validate_inbound_auth(self, auth_header: str, activity: dict[str, Any]) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            raise RuntimeError("PyJWT not installed. Run: pip install silver_research_bot-ai[msteams]")

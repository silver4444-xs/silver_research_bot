...
from silver_research_bot.agent.subagent import SubagentStatus
...
def _has_real_attr(obj: Any, key: str) -> bool:
    """判断一个对象是否真正（显式地）拥有某个属性，而不是由 mock 库自动生成的虚假属性."""
...
class MyTool(Tool):
    ...
    '''
    BLOCKED – 完全禁止访问（读写都不行）
    包含核心基础设施、敏感运行时状态、安全边界等，
    例如 bus、provider、_running、_mcp_servers、_pending_queues、_concurrency_gate 等。
    这些属性直接暴露会给系统带来安全风险或导致崩溃
    '''
    ...
    """
    READ_ONLY – 只读，不能修改
    例如 subagents（子代理管理器）、_current_iteration（只迭代计数）、exec_config、web_config（允许查看配置，但不能改变）。
    修改它们可能破坏系统一致性
    """
    ...
    """
    _DENIED_ATTRS – 永远禁止访问的 Python 特殊属性/方法
    如 __class__、__dict__、__bases__、__reduce__ 等，防止通过内省突破安全边界
    """
    ...
    """
    _SENSITIVE_NAMES – 敏感字段名（忽略大小写）
    包含 api_key, secret, password, token, credential, private_key 等。
    无论是作为属性名还是路径的一部分，这些字段都不会被暴露或修改，以防止泄露凭据。
    """
    ...
    def set_context(self, channel: str, chat_id: str) -> None:
        """保存当前会话的频道和聊天 ID，用于审计日志（_audit 方法）"""
    ...
    def _inspect(self, key: str | None) -> str:
        """根据路径 key 获取并格式化输出 AgentLoop 中的某个值"""
        if not key:
    ...
    def _modify(self, key: str | None, value: Any) -> str:
        """尝试修改 AgentLoop 的属性或草稿本中的键。受严格的访问控制"""
    ...
    def _modify_restricted(self, key: str, value: Any) -> str:
        """修改受限制但允许更改的配置项（如 max_iterations, context_window_tokens, model）"""
    ...
    def _validate_json_safe(cls, value: Any, depth: int = 0) -> str | None:
        """递归检查一个值是否可以被安全地 JSON 序列化（即不包含不可序列化的对象如 datetime、set、自定义类等）"""
        if depth > 10:

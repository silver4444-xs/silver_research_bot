...
from silver_research_bot import __logo__, __version__
from silver_research_bot.research_cli import research_app, paper_app
...
from silver_research_bot.cli.stream import StreamRenderer, ThinkingSpinner
from silver_research_bot.config.paths import get_workspace_path, is_default_workspace
from silver_research_bot.config.schema import Config
from silver_research_bot.utils.helpers import sync_workspace_templates
from silver_research_bot.utils.restart import (
...
app = typer.Typer(
    name="silver_research_bot",
    context_settings={"help_option_names": ["-h", "--help"]},
    help=f"{__logo__} silver_research_bot - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from silver_research_bot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} silver_research_bot[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} silver_research_bot v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """silver_research_bot - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize silver_research_bot configuration and workspace."""
    from silver_research_bot.config.loader import get_config_path, load_config, save_config, set_config_path
    from silver_research_bot.config.schema import Config
    ...
    if wizard:
        from silver_research_bot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'silver_research_bot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'silver_research_bot agent -m "Hello!"'
    gateway_cmd = "silver_research_bot gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} silver_research_bot is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/silver_research_bot#-chat-apps[/dim]"
    )
...
def _make_provider(config: Config):
    ...
    if backend == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.silver_research_bot/config.json under providers.azure_openai section")
        ...
        if needs_key and not exempt:
            ...
            console.print("Set one in ~/.silver_research_bot/config.json under providers section")
            raise typer.Exit(1)
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from silver_research_bot.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from silver_research_bot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show silver_research_bot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'silver_research_bot-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger
    from silver_research_bot.agent.loop import AgentLoop
    from silver_research_bot.api.server import create_app
    from silver_research_bot.bus.queue import MessageBus
    from silver_research_bot.session.manager import SessionManager

    if verbose:
        logger.enable("silver_research_bot")
    else:
        logger.disable("silver_research_bot")
...
def gateway(
    ...
    """Start the silver_research_bot gateway."""
    if verbose:
        import logging
...
def _run_gateway(
    ...
    *,
    ...
    from silver_research_bot.cron.types import CronJob
    from silver_research_bot.heartbeat.service import HeartbeatService
    from silver_research_bot.session.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting silver_research_bot gateway version {__version__} on port {port}...")
    ...
    from silver_research_bot.cron.types import CronJob, CronPayload
...
def web(
    ...
    from silver_research_bot.channels.manager import _default_webui_dist

    dist = _default_webui_dist()
    if dist is None:
        console.print(
            "[yellow]Warning: webui assets not found at silver_research_bot/web/dist/. "
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show silver_research_bot runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from silver_research_bot.channels.registry import discover_all
    from silver_research_bot.config.loader import load_config, set_config_path
...
def _get_bridge_dir() -> Path:
    ...
    from silver_research_bot.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # silver_research_bot/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall silver_research_bot")
        raise typer.Exit(1)
...
def plugins_list():
    ...
    from silver_research_bot.channels.registry import discover_all, discover_channel_names
...
@app.command()
def status():
    """Show silver_research_bot status."""
    from silver_research_bot.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} silver_research_bot Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from silver_research_bot.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from silver_research_bot.providers.github_copilot_provider import login_github_copilot

"""适用于固定 silver_research_bot 会话的 OpenAI 兼容 HTTP API 服务器。

提供 /v1/chat/completions 和 /v1/models 端点。
所有请求均路由至单个持久性 API 会话。
"""
...
from silver_research_bot.utils.helpers import safe_filename
...
async def handle_chat_completions(request: web.Request) -> web.Response:
    ...
    model_name: str = request.app.get("model_name", "silver_research_bot")
...
async def handle_models(request: web.Request) -> web.Response:
    """GET /v1/models"""
    model_name = request.app.get("model_name", "silver_research_bot")
    return web.json_response(
        {
            "object": "list",
            "data": [
...
def create_app(
    agent_loop, model_name: str = "silver_research_bot", request_timeout: float = 120.0
) -> web.Application:

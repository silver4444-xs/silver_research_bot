...
%%{init: {'theme':'base', 'themeVariables': { 'fontSize':'13px','fontFamily':'system-ui, sans-serif' }}}%%
flowchart TD
    BUS["📨 消息总线接收用户消息"]:::entry --> LOAD

    subgraph SESSION["💾 会话恢复"]
        LOAD["加载 / 创建会话"]:::session
        CKPT{"有运行时检查点？"}:::decision
        RESTORE["恢复未完成的消息<br/>助手回复 + 工具结果 + 报错占位符<br/>去重融合避免重复"]:::session
        PENDING{"有待处理用户消息标记？"}:::decision
    ...
    BUILD["🏗 上下文构建器组装消息"]:::context

    subgraph CTX["📋 上下文组装"]
        SYS["系统提示词<br/>身份 · 引导文件 · 记忆<br/>技能 · 近期历史"]:::context
        SUM{"自动压缩器<br/>空闲会话摘要？"}:::decision
    ...
    CONS{"Token 预算压缩器<br/>Token 超预算？"}:::decision
    COMPRESS["LLM 压缩旧消息块 → 摘要<br/>归档历史记录文件<br/>更新已压缩游标"]:::compress
    CONS -->|"是"| COMPRESS
    CONS -->|"否"| LOOP_START
    COMPRESS --> LOOP_START

    LOOP_START(["🔄 ReAct 循环开始<br/>迭代轮次 = 0"]):::loop

    subgraph GOVERN["🧹 五步上下文治理（每轮 LLM 调用前）"]
        direction TB
        G1["1️⃣ 清理孤儿结果<br/>移除无对应工具调用的结果消息"]:::govern
        G2["2️⃣ 缺失回填<br/>为已发出的工具调用补占位结果"]:::govern
        G3["3️⃣ 微压缩<br/>只读工具结果 > 500 字符<br/>保留最近 10 条，其余替换占位符"]:::govern
        G4["4️⃣ 工具结果截断<br/>单个结果不超过预设长度上限"]:::govern
        G5["5️⃣ 历史窗口裁剪<br/>Token 仍超限时裁剪最旧消息"]:::govern
        G1 --> G2 --> G3 --> G4 --> G5
    end

    LLM["🤖 LLM 调用<br/>提供商统一接口<br/>自动重试 · 错误分类 · 图片降级"]:::llm

    CHECK_RESP{"LLM 返回类型？"}:::decision

    subgraph TOOLS["🔧 工具执行"]
        SAVE_CKPT1["保存检查点<br/>阶段：工具待执行"]:::checkpoint
        EXEC["并行 / 串行执行工具<br/>重复调用拦截（同签名 > 3 次阻断）<br/>白名单 · 超时 · 沙箱"]:::tools
        SAVE_CKPT2["保存检查点<br/>阶段：工具已完成"]:::checkpoint
    ...
    subgraph FINAL["✅ 最终回复处理"]
        ...
        LENGTH{"结束原因 = 长度截断？"}:::decision
        RETRY_LENGTH["追加「继续」指令<br/>最多 3 次"]:::final
        DRAIN2{"中轮注入？"}:::decision
        APPEND["追加助手回复消息<br/>保存最终检查点<br/>阶段：最终回复"]:::final
    ...
    ERROR_STOP["❌ 错误终止<br/>工具错误 / LLM错误<br/>空最终回复"]:::error

    LOOP_START --> GOVERN
    GOVERN --> LLM
    LLM --> CHECK_RESP
    CHECK_RESP -->|"有工具调用"| TOOLS
    CHECK_RESP -->|"无工具调用"| FINAL
    DRAIN1 -->|"有注入"| GOVERN
    DRAIN1 -->|"无注入"| GOVERN
    ...
    DONE --> SAVE["💾 持久化本轮对话<br/>写入会话历史"]:::session
    SAVE --> PUB["📤 发布出站消息<br/>发送响应到消息总线"]:::session

    MAX_ITER{"达到最大迭代轮数？"}:::decision
...
> **关键防护机制**：① 重复工具调用拦截（同签名 > 3 次阻断） ② 最大迭代轮数硬上限（默认 20） ③ 空响应重试（最多 2 次） ④ 输出截断续写（最多 3 次） ⑤ 中轮注入循环限制（最多 5 周期）。检查点在三个边界保存（工具调用前 / 工具完成后 / 最终回复），崩溃恢复时自动重建消息并去重融合。
